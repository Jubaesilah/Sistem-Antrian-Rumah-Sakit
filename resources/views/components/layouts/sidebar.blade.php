            ...
            <div class="bg-gray-800 text-white w-64 h-screen flex flex-col fixed left-0 top-0 z-10">
                    ...
                    <a href="{{ route('dashboard') }}"
                        ...
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2V7zm0 0V5a2 2 0 012-2h6l2 2h6a2 2 0 012 2v2M7 13h10M7 17h10">
                            </path>
                        </svg>
                        Dashboard
                    </a>
                ...
                <div class="p-4 border-t border-gray-700">
                    ...
                    <form action="{{ route('logout') }}" method="POST" id="logoutForm"
                        onsubmit="return handleLogout(event)">
                        @csrf
                        <button type="submit" id="logoutBtn"
                            class="w-full flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-150 text-gray-300 hover:bg-red-600 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <script>
                    async function handleLogout(event) {
                        ...
                        // Call backend logout API in background
                        try {
                            const token = '{{ session('auth_token') }}';

                            if (token) {
                                // Call logout API (don't wait for response)
                                fetch('{{ env('BACKEND_API_URL', 'http://localhost:8000') }}/api/auth/logout', {
                                    method: 'POST',
                                    headers: {
                        ...
                        // Let form submit normally to clear session
                        return true;
                    }
                </script>
            </div>
